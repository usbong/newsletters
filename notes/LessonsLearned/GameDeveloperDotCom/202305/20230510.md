...
特にFLASHの場合に関しては、一部のゲームを保存し、将来的に遊び続けるようにするために、多様な活動があると。例えばインターネット・アーカイブにおいて、APPLE IIゲームを遊べるエミュレートがあり、RUFFLEというFLASHエミュレータも、ウェブプラグインとして使われると。だが、このゲームを保存するには、多量の作業である。Kongregateだけだと、10万本超が存在しており、Newgroundsだと、アニメも含めて、指数関数的に増える膨大な数だと。
...
Macromedia Director 6、小学校〜中学校の時に初めて習った。このソフトによってインターアクティブ・ストリーブックやアニメを作ることができた。当時、GIF（アニメ画像）が、すでに広がってきたが、パワーポイントよりアニメのような作品ができるソフトエアツールだと考えていた。何枚のフレームを利用できるし、リンク機能も追加できる中で、牧場物語（SFC)のスプライト画像を入れてみたら、トラックの車が、幾つかの牧場の背景で走っていたアニメを作れた。当時SFC・SNESエミュレーターがすでに存在していた時代だが、インタネットにおいて何本拾えるゲームのように牧場物語は、試行までしか実行しなかった。アメリカでは、カセットを持っていると、ソフトROMを使えるルールがあると、何人の人たちが言うけれど、自分に対してグレイとなる場合、使わないほうが良い。<br/>
<br/>
